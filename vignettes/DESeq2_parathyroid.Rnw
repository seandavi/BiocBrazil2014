...
% library('cacheSweave'); rm(list=ls()); Sweave('DESeq2_parathyroid.Rnw',driver=cacheSweaveDriver()); system('pdflatex DESeq2_parathyroid'); relocateAnswers(); for(i in 1:2) system('pdflatex DESeq2_parathyroid')
...
Here we use a locally cached copy of \Robject{ucscTable}:

<<localERBS>>=
ucscTableFile <- "localUcscTable.csv"
ucscTable <- read.csv(ucscTableFile, stringsAsFactors=FALSE)
@

We now can use the downloaded table of annotated estrogen receptor peaks.
